function UCloudUFile(bucketName, bucketUrl, tokenPublicKey, tokenPrivateKey, tokenServerUrl, prefix) {
    ...
    this.bucketName = bucketName || "rongyaomedia-sit";

    // 存储空间域名。既可以在这里配置，也可以在实例化时传参配置。
    // 例如 bucketUrl = "https://example-name.cn-bj.ufileos.com/"
    this.bucketUrl = bucketUrl || "https://rongyaomedia-sit.cn-bj.ufileos.com";

    // 令牌公钥。既可以在这里配置，也可以在实例化时传参配置。
    this.PUBLIC_KEY = tokenPublicKey || 'TOKEN_f478d191-f390-4c0a-bf8e-40e72f04c6b4';

    // 令牌私钥。既可以在这里配置，也可以在实例化时传参配置。
    this.PRIVATE_KEY = tokenPrivateKey || 'c95ac398-6eed-48ad-a5a9-e61fe79e9df4';
